...
success-criteria:
  ...
  - Running `./setup.sh` on a fresh Linux Mint or macOS machine walks the user through full node enrollment with no prior knowledge of wgmesh or WireGuard.
...
The existing `wgmesh bootstrap` command (SSH-executed shell scripts) served as a
...
- TUI flows for: prerequisite installation (Linux Mint and macOS), secret
  ...
  and node enrollment (wgmesh add + sync + service file installation).
...
- Full Ansible inventory management for non-hub nodes (node config is handled by
  wgmesh CLI + service file templates, not Ansible).
...
- **SPEC-003 (Implemented):** WireGuard Hub & Network — the wgmesh CLI and
