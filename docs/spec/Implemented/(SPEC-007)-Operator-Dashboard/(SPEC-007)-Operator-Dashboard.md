...
| Implemented | 2026-03-03 | 6738203 | porthole dashboard command, port 8080 |
...
The existing `porthole status` CLI is powerful but requires SSH context and
text parsing.
...
1. `porthole dashboard` starts a local HTTP server on `0.0.0.0:8080`.
...
5. `porthole dashboard --port <N>` changes the listening port.

## Implementation Approach

- `src/porthole/commands/dashboard.py` with `run_dashboard(port)`.
...
- `porthole dashboard` command added to `cli.py`.

## Success Criteria

- `porthole dashboard` starts without error and serves on port 8080.
