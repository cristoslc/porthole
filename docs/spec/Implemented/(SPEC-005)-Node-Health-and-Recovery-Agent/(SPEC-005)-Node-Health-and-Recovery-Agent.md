...
7. When the watchdog **successfully reconnects** a tunnel (WireGuard restart
   succeeded and hub ping passes), it logs a structured reconnect event:
   `porthole-watchdog: reconnected peer=<name> after=<N>s`. This enables
   `porthole status` to display "reconnected N minutes ago" alongside the
   handshake timestamp, giving the operator visibility into recovery events
   (addresses JOURNEY-004.PP-03).
...
- **Windows**: Task Scheduler task with 2-minute repeat trigger. PowerShell
  health-check script (`wg-watchdog.ps1`). Wake trigger for post-sleep
  recovery. Generated by `porthole gen-peer-scripts` alongside the Linux/macOS
  scripts. **Note:** Windows watchdog templates are not yet implemented in
  `gen-peer-scripts` — this is a known code gap vs. this spec. The template
  `templates/peer-scripts/wg-watchdog.ps1.j2` and its Task Scheduler XML
  export need to be added (addresses JOURNEY-003.PP-04).
