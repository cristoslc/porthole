...
| EPIC-002 (Provisioning CLI) | **Absorb** into new Hub + Node Agent epics | `porthole` CLI concept lives on, but "provisioning" splits: hub-side sync goes to Hub Bootstrap, node-side provisioning goes to Node Agent |
...
**Success criteria:**
- `porthole hub deploy` (or `hub-up.sh`) against any cloud provider creates a
...
- Operator runs `porthole add <name>` on their workstation → generates config
...
- Generates SSH config (`~/.ssh/config.d/porthole`) with ProxyJump entries for
...
**Success criteria:**
- New node onboarded in <5 minutes (from `porthole add` to connected tunnel)
...
- C. TUI subcommand in the node agent (`porthole monitor`)
...
EPIC-007 (Hub Bootstrap)
  ...
  │
  └── porthole CLI — shared tooling between hub deploy and node provisioning
```

The `porthole` CLI is not its own epic — it's the shared tool that both
...
1. `porthole add` + Magic Wormhole transfer + manual WireGuard install/config
...
2. **`porthole` as a single binary?** Should the CLI, node agent, and TUI be
   one binary invoked in different modes (`porthole hub deploy`, `porthole agent
   run`, `porthole tui`)? Or separate packages?
